...
parser.add_argument('-c', '--columns', help='Print all the columns in the input file.')
...
columns = dataframe['CAMPOS_DEF_USUARIO']
...
try:
    for index, column in enumerate(columns):
        ...
        if pd.notna(column):
            ...
            dataframe.at[index, 'CAMPOS_DEF_USUARIO'] = extracted_value
        else :
            dataframe.at[index, 'CAMPOS_DEF_USUARIO'] = '[Vacio]'
except Exception as e:
    print(f'Unexpected error processing excel file: {e}')

# Save contents to Excel file
dataframe.to_excel('CLIENTES_MOD.xlsx', index=False)
