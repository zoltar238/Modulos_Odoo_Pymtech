    ...
    def import_csv(self):
        if self.import_option == 'xls':
            try:
                ...
                raise exceptions.RedirectWarning(_("Invalid file ! "),action='#', button_text=_("Go Back"))
            ...
            for line in dict_list:
                lot = line.get('Serial')
                prod_lot_id = self.env['stock.lot']
                product_id = self.env['product.product'].sudo().search([('name', '=', line.get('Product').split(']')[1][1:])])   

                if not product_id:
                    raise exceptions.RedirectWarning(_("Product %s does not exist !" % line.get('Product')),action='#', button_text=_("Go Back"))
                if lot:
                    prod_lot_id = self.env['stock.lot'].sudo().create({'name': lot, 'product_id': product_id.id, 'company_id': self.env.user.company_id.id})
                    if not prod_lot_id:
                        raise exceptions.RedirectWarning(_("Lot %s does not exist !" % line.get('Serial')),action='#', button_text=_("Go Back") )
                # if not inventory_obj:
                #     inventory_obj = inventory_obj.create({'product_id':self.inv_name,'location_ids':self.location_id.ids, 'date': self.date})
                    # inventory_obj.action_start()

                stock_line_id = self.env['stock.quant'].sudo().create({
                    'in_date': self.date,
                    'product_id': product_id.id,
                    'location_id': self.location_id.id,
                    'lot_id': prod_lot_id and prod_lot_id.id,
