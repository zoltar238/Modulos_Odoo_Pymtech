...
class HrAttendance(models.Model):
    ...
    def _compute_overtime_hours(self):
        ...
        if self.employee_id:
            ...
            self.env.cr.execute('''
                                WITH employee_time_zones AS (SELECT employee.id AS employee_id,
        ...
        for company in all_companies:
            ...
            to_check_out = to_verify_company.filtered(
                lambda a: (fields.Datetime.now() - a.check_in).seconds / 3600 + mapped_previous_duration[a.employee_id][
                    a.check_in.date()] - max_tol > (sum(a.employee_id.resource_calendar_id.attendance_ids.filtered(
                    lambda att: att.dayofweek == str(a.check_in.weekday())).mapped('duration_hours'))))
            body = _(
            ...
            for att in to_check_out:
                delta_duration = max(1, (sum(att.employee_id.resource_calendar_id.attendance_ids.filtered(
                    lambda a: a.dayofweek == str(att.check_in.weekday())).mapped('duration_hours')) + max_tol -
                                         mapped_previous_duration[att.employee_id][att.check_in.date()]) * 3600)
                att.write({
